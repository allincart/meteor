...
describe("mt-url-field", () => {
  ...
  it("updates the domain when the user types", async () => {
    ...
    // ACT
    await userEvent.type(screen.getByRole("textbox"), "www.allincart.cn");

    // ASSERT
    expect(screen.getByRole("textbox")).toHaveValue("www.allincart.cn");

    expect(handler).toHaveBeenCalledTimes(16);
    expect(handler).toHaveBeenNthCalledWith(1, "https://w");
    expect(handler).toHaveBeenNthCalledWith(16, "https://www.allincart.cn");
  });
  ...
  it("updates the domain when the user types and then focuses another element", async () => {
    ...
    expect(handler).toHaveBeenCalledWith("www.allincart.cn");
  });

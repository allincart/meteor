...
describe("mt-textarea", () => {
  ...
  it("emits the changed value when the user moves focus to another element", async () => {
    ...
    // ACT
    await userEvent.type(screen.getByRole("textbox"), "AllinCart");
    await userEvent.tab();

    // ASSERT
    expect(handler).toHaveBeenNthCalledWith(1, "AllinCart");
  });
  ...
  it("does not allow typing more characters than the specified max length", async () => {
    ...
    // ASSERT
    expect(screen.getByRole("textbox")).toHaveValue("Alli");
    expect(handler).toHaveBeenNthCalledWith(4, "Alli");
  });
  ...
  it("is possible to edit value of textarea with unlinked inheritance", async () => {
    ...
    // ASSERT
    expect(screen.getByRole("textbox")).toHaveValue("AllinCart");
